...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		{
			$query .= '
			ORDER BY '.strval((int)$_POST['order']['0']['column'] + 1).' '.$_POST['order']['0']['dir'].' 
			';
		}
		else
		{	
	...
	{
		$query = "
		SELECT tbl_student.student_id, tbl_student.student_name, tbl_student.student_roll_number, tbl_grade.grade_name, tbl_teacher.teacher_name FROM tbl_student 
		...
		$query .= 'GROUP BY tbl_student.student_id, tbl_teacher.teacher_name ';
		if(isset($_POST["order"]))
		{
			$query .= 'ORDER BY '.strval((int)$_POST['order']['0']['column'] + 1).' '.$_POST['order']['0']['dir'].' ';
		}
		else
		{
		...
		*/
		// file_put_contents("data_table.txt", $query_status);
		$result = $statement->fetchAll();
		// file_put_contents("data_table.txt", print_r($result, true), FILE_APPEND);
		$data = array();
		$filtered_rows = $statement->rowCount();
		foreach($result as $row)
		{
		...
		// file_put_contents("data_table.txt", print_r($output, true), FILE_APPEND);
		echo json_encode($output);
	}
}
